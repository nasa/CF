...
void TestUtil_InitMsg(CFE_MSG_Message_t *MsgPtr, 
...
int Any_int_GreaterThan(int floor) /* NOTE: INTMAX_MAX will fail, and is invalid value */
{
  ...
  {  
    ...
    {
        random_val = (int) (rand() % (INTMAX_MAX)); /* floor is negative, random will be positive so any positive (or zero) will work */
    }
  }

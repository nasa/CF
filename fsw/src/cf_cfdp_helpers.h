...
#define CF_BSWAP32(x) ((uint32)((((x) & 0xff000000u)>>24) | (((x) & 0x00ff0000u) >> 8) | (((x) & 0x0000ff00u) << 8) | (((x) & 0x000000ffu) << 24)))
#define CF_BSWAP64(x) ((uint64)((((x) & 0xff00000000000000)>>56) | (((x) & 0x00ff000000000000)>>48) | (((x) & 0x0000ff0000000000)>>40) | (((x) & 0x000000ff00000000)>>32) | (((x) & 0xff000000u)>>24) | (((x) & 0x00ff0000u) >> 8) | (((x) & 0x0000ff00u) << 8) | (((x) & 0x000000ffu) << 24)))
...
#define DECL_LDST(type, dst, src, efn) do { (dst) = efn((src)); } while(0)
#define DECL_LDST_NOSWAP(type, dst, src) do { (dst) = (src); } while(0)
#else
#define DECL_LDST(type, dst, src, efn) do { \
    type t_____s = efn((src)); \
    memcpy(&(dst), &t_____s, sizeof(t_____s)); \
    } \
    while (0)
#define DECL_LDST_NOSWAP(type, dst, src) do { \
    type t_____s = (src); \
    memcpy(&(dst), &t_____s, sizeof(t_____s)); \
    } \
    while (0)
...
#define cfdp_set_uint8(dst, src) do { (dst) = (src); } while(0)
...
#define cfdp_get_uint8(dst, src) do { (dst) = (src); } while(0)
...
#define cfdp_ldst_uint8(dst, src) do { (dst) = (src); } while(0)

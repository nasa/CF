...
static int CF_CFDP_R_CheckCrc(transaction_t *t, uint32 expected_crc)
{
    ...
    if(t->crc.result!=expected_crc) {
        CFE_EVS_SendEvent(CF_EID_ERR_CFDP_R_CRC, CFE_EVS_EventType_ERROR, "CF R%d(%u:%u): crc mismatch for R trans. got 0x%x expected 0x%x", (t->state==CFDP_R2), t->history->src_eid, t->history->seq_num, t->crc.result, expected_crc);
...
static void CF_CFDP_R2_Complete(transaction_t *t, int ok_to_send_nak)
{
    ...
    if(send_nak&&ok_to_send_nak) {
        if(++t->state_data.r.r2.counter.nak==CF_AppData.config_table->nak_limit) {
            CFE_EVS_SendEvent(CF_EID_ERR_CFDP_R_NAK_LIMIT, CFE_EVS_EventType_ERROR, "CF R%d(%u:%u): nak limited reach", (t->state==CFDP_R2), t->history->src_eid, t->history->seq_num);
            send_fin = 1;
            ++CF_AppData.hk.channel_hk[t->chan_num].counters.fault.nak_limit;
            t->history->cc = CC_NAK_LIMIT_REACHED; /* don't use CF_CFDP_R2_SetCc because many places in this function set send_fin */
    ...
    *bytes_received -= CF_HeaderSize(ph);
    ...
    if(*bytes_received>sizeof(pdu_file_data_header_t)) {
        ...
        if(t->state_data.r.cached_pos!=offset) {
            ...
            if(fret!=offset) {
                CFE_EVS_SendEvent(CF_EID_ERR_CFDP_R_SEEK_FD, CFE_EVS_EventType_ERROR, "CF R%d(%u:%u): failed to seek offset %u, got 0x%08x", (t->state==CFDP_R2), t->history->src_eid, t->history->seq_num, offset, fret);
        ...
        *bytes_received -= sizeof(pdu_file_data_header_t);
        ...
        if(fret!=*bytes_received) {
            CFE_EVS_SendEvent(CF_EID_ERR_CFDP_R_WRITE, CFE_EVS_EventType_ERROR, "CF R%d(%u:%u): OS_write returned 0x%08x, got 0x%08x", (t->state==CFDP_R2), t->history->src_eid, t->history->seq_num, offset, fret);
...
static int CF_CFDP_R_SubstateRecvEof(transaction_t *t, const pdu_header_t *ph)
{
    ...
    if(!CF_CFDP_RecvEof()) {
        ...
        if(t->flags.rx.md_recv&&(size!=t->fsize)) {
            CFE_EVS_SendEvent(CF_EID_ERR_CFDP_R_SIZE_MISMATCH, CFE_EVS_EventType_ERROR, "CF R%d(%u:%u): eof file size mismatch: got %u expected %u", (t->state==CFDP_R2), t->history->src_eid, t->history->seq_num, size, t->fsize);
        ...
        CFE_EVS_SendEvent(CF_EID_ERR_CFDP_R_PDU_EOF, CFE_EVS_EventType_ERROR, "CF R%d(%u:%u): invalid eof packet", (t->state==CFDP_R2), t->history->src_eid, t->history->seq_num);
...
static void CF_CFDP_R2_GapCompute(const chunks_t *chunks, const chunk_t *c, void *opaque)
{
    ...
    cfdp_ldst_uint32(nak->segment_requests[args->gap_counter].offset_end, (nak->segment_requests[args->gap_counter].offset_start+c->size));
    ++args->gap_counter;
}
...
{
    pdu_header_t *ph = CF_CFDP_ConstructPduHeader(t, PDU_NAK, t->history->peer_eid, CF_AppData.config_table->local_eid, 1, t->history->seq_num, 1);
    ...
    if(ph) {
        if(t->flags.rx.md_recv) {
            ...
            cret = CF_Chunks_ComputeGaps(&t->chunks->chunks, (t->chunks->chunks.count<t->chunks->chunks.CF_max_chunks)?t->chunks->chunks.CF_max_chunks:(t->chunks->chunks.CF_max_chunks-1), t->fsize, 0, CF_CFDP_R2_GapCompute, &args);
            ...
            if(!cret) {
                ...
                CF_Assert(sret!=CF_SEND_ERROR); /* NOTE: this CF_Assert is here because CF_CFDP_SendNak() does not return CF_SEND_ERROR, so if it's ever added to that function we need to test handling it here */
            ...
            CFE_EVS_SendEvent(CF_EID_INF_CFDP_R_REQUEST_MD, CFE_EVS_EventType_INFORMATION, "CF R%d(%u:%u): requesting MD", (t->state==CFDP_R2), t->history->src_eid, t->history->seq_num);
            ...
            CF_Assert(sret!=CF_SEND_ERROR); /* this CF_Assert is here because CF_CFDP_SendNak() does not return CF_SEND_ERROR */
            if(sret==CF_SEND_SUCCESS) {
                ret = 0;
            }
        }
...
void CF_CFDP_R_Init(transaction_t *t)
{
    ...
    if(t->state==CFDP_R2) {
        if(!t->flags.rx.md_recv) {
            ...
            snprintf(t->history->fnames.dst_filename, sizeof(t->history->fnames.dst_filename)-1, "%.*s/%d.tmp", CF_FILENAME_MAX_PATH-1, CF_AppData.config_table->tmp_dir, t->history->seq_num);
            CFE_EVS_SendEvent(CF_EID_INF_CFDP_R_TEMP_FILE, CFE_EVS_EventType_INFORMATION, "CF R%d(%u:%u): making temp file %s for transaction without MD", (t->state==CFDP_R2), t->history->src_eid, t->history->seq_num, t->history->fnames.dst_filename);
        }
    ...
    if(ret<0) {
        CFE_EVS_SendEvent(CF_EID_ERR_CFDP_R_CREAT, CFE_EVS_EventType_ERROR, "CF R%d(%u:%u): failed to create file %s for writing, error=0x%08x", (t->state==CFDP_R2), t->history->src_eid, t->history->seq_num, t->history->fnames.dst_filename, ret);
...
#define RXC t->state_data.r.r2.rx_crc_calc_bytes
    ...
    while((count_bytes<CF_AppData.config_table->rx_crc_calc_bytes_per_wakeup)&&(RXC<t->fsize)) {
        ...
        if(t->state_data.r.cached_pos!=RXC) {
            ...
            if(fret!=RXC) {
                CFE_EVS_SendEvent(CF_EID_ERR_CFDP_R_SEEK_CRC, CFE_EVS_EventType_ERROR, "CF R%d(%u:%u): failed to seek offset %u, got 0x%08x", (t->state==CFDP_R2), t->history->src_eid, t->history->seq_num, RXC, fret);
        ...
        if(fret!=read_size) {
            CFE_EVS_SendEvent(CF_EID_ERR_CFDP_R_READ, CFE_EVS_EventType_ERROR, "CF R%d(%u:%u): failed to read file expected %u, got 0x%08x", (t->state==CFDP_R2), t->history->src_eid, t->history->seq_num, read_size, fret);
            t->history->cc = CC_FILE_SIZE_ERROR; /* should be ok to use this one */
...
static void CF_CFDP_R2_Recv_fin_ack(transaction_t *t, const pdu_header_t *ph)
{
    if(!CF_CFDP_RecvAck()) {
        ...
        CFE_EVS_SendEvent(CF_EID_ERR_CFDP_R_PDU_FINACK, CFE_EVS_EventType_ERROR, "CF R%d(%u:%u): invalid fin-ack", (t->state==CFDP_R2), t->history->src_eid, t->history->seq_num);
        ++CF_AppData.hk.channel_hk[t->chan_num].counters.recv.error;
    }
}
...
static void CF_CFDP_R2_RecvMd(transaction_t *t, const pdu_header_t *ph)
{
    ...
    if(!t->flags.rx.md_recv) {
        ...
        strcpy(fname, t->history->fnames.dst_filename); /* strcpy is ok, since fname is CF_FILENAME_MAX_LEN like dst_filename */
        ...
        if(!status) {
            ...
            if(t->flags.rx.eof_recv) {
                ...
                if(t->state_data.r.r2.eof_size!=t->fsize) {
                    CFE_EVS_SendEvent(CF_EID_ERR_CFDP_R_EOF_MD_SIZE, CFE_EVS_EventType_ERROR, "CF R%d(%u:%u): eof/md size mismatch md: %d, eof: %d", (t->state==CFDP_R2), t->history->src_eid, t->history->seq_num, t->fsize, t->state_data.r.r2.eof_size);
                    ++CF_AppData.hk.channel_hk[t->chan_num].counters.fault.file_size_mismatch;
                    CF_CFDP_R2_SetCc(t, CC_FILE_SIZE_ERROR);
                    goto err_out;
                }
            ...
            if(status!=OS_SUCCESS) {
                CFE_EVS_SendEvent(CF_EID_ERR_CFDP_R_RENAME, CFE_EVS_EventType_ERROR, "CF R%d(%u:%u): failed to rename file in R2, error=0x%08x", (t->state==CFDP_R2), t->history->src_eid, t->history->seq_num, status);
                ...
                int32 ret = CF_WrappedOpenCreate(&t->fd, t->history->fnames.dst_filename, OS_FILE_FLAG_NONE, OS_READ_WRITE);
                if(ret<0) {
                    CFE_EVS_SendEvent(CF_EID_ERR_CFDP_R_OPEN, CFE_EVS_EventType_ERROR, "CF R%d(%u:%u): failed to open renamed file in R2, error=0x%08x", (t->state==CFDP_R2), t->history->src_eid, t->history->seq_num, ret);
                    CF_CFDP_R2_SetCc(t, CC_FILESTORE_REJECTION);  
                    ++CF_AppData.hk.channel_hk[t->chan_num].counters.fault.file_open;
                    t->fd = OS_OBJECT_ID_UNDEFINED; /* just in case */
                    goto err_out;
            ...
            CFE_EVS_SendEvent(CF_EID_ERR_CFDP_R_PDU_MD, CFE_EVS_EventType_ERROR, "CF R%d(%u:%u): invalid md received", (t->state==CFDP_R2), t->history->src_eid, t->history->seq_num);
            ++CF_AppData.hk.channel_hk[t->chan_num].counters.recv.error;
...
**
*************************************************************************/
static void CF_CFDP_R_DispatchRecv(transaction_t *t, void (*const fns[RECV_NUM_STATES][PDU_INVALID_MAX])(transaction_t*, const pdu_header_t*), void (*const fd_fn)(transaction_t*, const pdu_header_t*))
{
    ...
    if(!FGV(ph->flags, PDU_HDR_FLAGS_TYPE)) {
        ...
        if(fdh->directive_code<PDU_INVALID_MAX) {
            ...
            CFE_EVS_SendEvent(CF_EID_ERR_CFDP_R_DC_INV, CFE_EVS_EventType_ERROR, "CF R%d(%u:%u): received pdu with invalid directive code %d for sub-state %d", (t->state==CFDP_R2), t->history->src_eid, t->history->seq_num, fdh->directive_code, t->state_data.r.sub_state);
        }
    }
    else {
        if(t->history->cc==CC_NO_ERROR) {
...
void CF_CFDP_R1_Recv(transaction_t *t)
{
    static void (* const substate_fns[RECV_NUM_STATES][PDU_INVALID_MAX])(transaction_t *t, const pdu_header_t*) = {
        ...
        { NULL, NULL, NULL, NULL, CF_CFDP_R1_SubstateRecvEof, NULL, NULL, NULL, NULL, NULL, NULL }, /* RECV_WAIT_FOR_FIN_ACK */
    };
...
{
    static void (* const substate_fns[RECV_NUM_STATES][PDU_INVALID_MAX])(transaction_t *t, const pdu_header_t*) = {
        { NULL, NULL, NULL, NULL, CF_CFDP_R2_SubstateRecvEof, NULL, NULL, CF_CFDP_R2_RecvMd, NULL, NULL, NULL }, /* RECV_FILEDATA */
        { NULL, NULL, NULL, NULL, CF_CFDP_R2_SubstateRecvEof, NULL, NULL, CF_CFDP_R2_RecvMd, NULL, NULL, NULL }, /* RECV_EOF */
        { NULL, NULL, NULL, NULL, CF_CFDP_R2_SubstateRecvEof, NULL, CF_CFDP_R2_Recv_fin_ack, NULL, NULL, NULL, NULL }, /* RECV_WAIT_FOR_FIN_ACK */
    };
...
{
    CFE_EVS_SendEvent(CF_EID_ERR_CFDP_R_INACT_TIMER, CFE_EVS_EventType_ERROR, "CF R%d(%u:%u): inactivity timer expired", (t->state==CFDP_R2), t->history->src_eid, t->history->seq_num);
...
void CF_CFDP_R_Tick(transaction_t *t, int *cont /* unused */)
{
    ...
    if(t->state==CFDP_R2) {
        ...
        if(t->flags.rx.send_ack) {
            cfdp_send_ret_t sret = CF_CFDP_SendAck(t, ACK_TS_ACTIVE, PDU_EOF, t->state_data.r.r2.eof_cc, t->history->peer_eid, t->history->seq_num);
            ...
            if(CF_Timer_Expired(&t->ack_timer)) {
                ...
                if(!t->flags.rx.complete) {
                    ...
                    if(++t->state_data.r.r2.counter.ack==CF_AppData.config_table->ack_limit) {
                        CFE_EVS_SendEvent(CF_EID_ERR_CFDP_R_ACK_LIMIT, CFE_EVS_EventType_ERROR, "CF R2(%u:%u): ack limit reached, no fin-ack", t->history->src_eid, t->history->seq_num);

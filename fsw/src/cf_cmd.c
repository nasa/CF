...
{
    CFE_EVS_SendEvent(CF_EID_INF_CMD_NOOP, CFE_EVS_EventType_INFORMATION, 
                      "CF: No-Op received, Version %d.%d.%d",
                      CF_MAJOR_VERSION,
                      CF_MINOR_VERSION,
                      CF_REVISION);
    CF_CmdAcc();
...
static void CF_CmdReset(CFE_SB_Buffer_t *msg)
{
    ...
    if(param>4) {
        CFE_EVS_SendEvent(CF_EID_ERR_CMD_RESET_INVALID, CFE_EVS_EventType_ERROR, "CF: Received RESET COUNTERS command with invalid parameter %d", param);
        CF_CmdRej();
        goto err_out;
    }

    CFE_EVS_SendEvent(CF_EID_INF_CMD_RESET, CFE_EVS_EventType_INFORMATION, "CF: Received RESET COUNTERS command: %s", names[param]);
...
static void CF_CmdTxFile(CFE_SB_Buffer_t *msg)
{
    ...
    CF_CmdCond(CF_CFDP_TxFile(tx->src_filename, tx->dst_filename, tx->cfdp_class, tx->keep, tx->chan_num, tx->priority, tx->dest_id));
}
...
static void CF_CmdPlaybackDir(CFE_SB_Buffer_t *msg)
{
    ...
    CF_CmdCond(CF_CFDP_PlaybackDir(tx->src_filename, tx->dst_filename, tx->cfdp_class, tx->keep, tx->chan_num, tx->priority, tx->dest_id));
}
...
static int CF_DoChanAction(cf_cmd_unionargs_t *cmd, const char *errstr, chan_action_fn_t fn, void *context)
{
    ...
    if(cmd->data.byte[0]==ALL_CHANNELS) {
        ...
        int i;
        for(i=0; i<CF_NUM_CHANNELS; ++i) ret |= fn(i, context);
    }
        ...
        /* bad parameter */
        CFE_EVS_SendEvent(CF_EID_ERR_CMD_CHAN_PARAM, CFE_EVS_EventType_ERROR, "CF: %s: channel parameter out of range. received %d", errstr, cmd->data.byte[0]);
        ret = -1;
    }
...
static int CF_TsnChanAction(cf_cmd_transaction_t *cmd, const char *cmdstr, CF_TsnChanAction_fn_t fn, void *context)
{
    ...
    if(cmd->chan==COMPOUND_KEY) {
        ...
        if(t) {
            ...
            CFE_EVS_SendEvent(CF_EID_ERR_CMD_TRANS_NOT_FOUND, CFE_EVS_EventType_ERROR, "CF: %s cmd: failed to find transactino for (eid %d, ts %d)", cmdstr, cmd->eid, cmd->ts);
        }
    }
        ...
        CFE_EVS_SendEvent(CF_EID_ERR_CMD_TSN_CHAN_INVALID, CFE_EVS_EventType_ERROR, "CF: %s cmd: invalid channel %d", cmdstr, cmd->chan);
    }
...
static void CF_DoSuspRes(cf_cmd_transaction_t *cmd, uint8 action)
{
    ...
    if(!ret&&args.same) {
        CFE_EVS_SendEvent(CF_EID_ERR_CMD_SUSPRES_SAME, CFE_EVS_EventType_ERROR, "CF: %s cmd: setting suspend flag to current value of %d", msgstr[action], action);
        CF_CmdRej();
    }
    else if(ret) {
        CFE_EVS_SendEvent(CF_EID_ERR_CMD_SUSPRES_CHAN, CFE_EVS_EventType_ERROR, "CF: %s cmd: no transaction found", msgstr[action]);
        CF_CmdRej();
    }
    else {
        CF_CmdAcc();
...
{
    bool_arg_t barg = { 1 };
    CF_CmdCond(CF_DoChanAction((cf_cmd_unionargs_t*)msg, "enable_dequeue", (chan_action_fn_t)CF_DoEnableDisableDequeue, &barg));
}
...
{
    bool_arg_t barg = { 0 };
    CF_CmdCond(CF_DoChanAction((cf_cmd_unionargs_t*)msg, "disable_dequeue", (chan_action_fn_t)CF_DoEnableDisableDequeue, &barg));
}
...
static int CF_DoEnableDisablePolldir(uint8 chan_num, const bool_msg_arg_t *context)
{
    ...
    if(context->msg->data.byte[1]==ALL_POLLDIRS) {
        ...
        int i;
        for(i=0; i<CF_MAX_POLLING_DIR_PER_CHAN; ++i) CF_AppData.config_table->chan[chan_num].polldir[i].enabled = context->barg;
    }
        ...
        CFE_EVS_SendEvent(CF_EID_ERR_CMD_POLLDIR_INVALID, CFE_EVS_EventType_ERROR, "CF: enable/disable polldir: invalid polldir %d on channel %d", context->msg->data.byte[1], chan_num);
        ret = -1;
    }
...
static void CF_CmdEnablePolldir(CFE_SB_Buffer_t *msg)
{
    ...
    CF_CmdCond(CF_DoChanAction((cf_cmd_unionargs_t*)msg, "enable_polldir", (chan_action_fn_t)CF_DoEnableDisablePolldir, &barg));
}
...
static void CF_CmdDisablePolldir(CFE_SB_Buffer_t *msg)
{
    ...
    CF_CmdCond(CF_DoChanAction((cf_cmd_unionargs_t*)msg, "disable_polldir", (chan_action_fn_t)CF_DoEnableDisablePolldir, &barg));
}
...
static int CF_DoPurgeQueue(uint8 chan_num, cf_cmd_unionargs_t *cmd)
{
    ...
    switch(cmd->data.byte[1]) {
        ...
        default:
            CFE_EVS_SendEvent(CF_EID_ERR_CMD_PURGE_ARG, CFE_EVS_EventType_ERROR, "CF: purge queue invalid arg %d", cmd->data.byte[1]);
            ret = -1;
        break;
    }
...
static void CF_CmdWriteQueue(CFE_SB_Buffer_t *msg)
{
    ...
    if((wq->type==type_up)&&(wq->queue==q_pend)) {
        CFE_EVS_SendEvent(CF_EID_ERR_CMD_WQ_ARGS, CFE_EVS_EventType_ERROR, "CF: write queue invalid command parameters");
        goto bail;
    }

    /* PTFO: queues can be large. may want to split this work up across the state machine and take several wakeups to complete */
    ret = CF_WrappedOpenCreate(&fd, wq->filename, OS_FILE_FLAG_CREATE, OS_WRITE_ONLY);
    if(ret<0) {
        CFE_EVS_SendEvent(CF_EID_ERR_CMD_WQ_OPEN, CFE_EVS_EventType_ERROR, "CF: write queue failed to open file %s", wq->filename);
        goto bail;
    }
    ...
    if(!wq->type||(wq->type==type_up)) {
        ...
        if((wq->queue==q_all)||(wq->queue==q_active)) {
            ...
            if(ret) {
                CFE_EVS_SendEvent(CF_EID_ERR_CMD_WQ_WRITEQ_RX, CFE_EVS_EventType_ERROR, "CF: write queue failed to write CF_Q_RX data");
                goto out_close;
            }
        }
        ...
        if((wq->queue==q_all)||(wq->queue==q_history)) {
            ...
            if(ret) {
                CFE_EVS_SendEvent(CF_EID_ERR_CMD_WQ_WRITEHIST_RX, CFE_EVS_EventType_ERROR, "CF: write queue failed to write history RX data");
                goto out_close;
            }
        }
    }
    ...
    if(!wq->type||(wq->type==type_down)) {
        ...
        if((wq->queue==q_all)||(wq->queue==q_active)) {
            ...
            for(i=0; i<2; ++i) {
                ...
                if(ret) {
                    CFE_EVS_SendEvent(CF_EID_ERR_CMD_WQ_WRITEQ_TX, CFE_EVS_EventType_ERROR, "CF: write queue failed to write q index %d", qs[i]);
                    goto out_close;
                }
            }
        }
        ...
        if((wq->queue==q_all)||(wq->queue==q_pend)) {
            ...
            if(ret) {
                CFE_EVS_SendEvent(CF_EID_ERR_CMD_WQ_WRITEQ_PEND, CFE_EVS_EventType_ERROR, "CF: write queue failed to write pending queue");
                goto out_close;
            }
        }
        ...
        if((wq->queue==q_all)||(wq->queue==q_history)) {
            ...
            if(ret) {
                CFE_EVS_SendEvent(CF_EID_ERR_CMD_WQ_WRITEHIST_TX, CFE_EVS_EventType_ERROR, "CF: write queue failed to write CF_Q_TX data");
                goto out_close;
            }
        }
    }
...
static void CF_CmdGetSetParam(uint8 is_set, uint8 param_id, uint32 value, uint8 chan_num)
{
     ...
     * access. */
#define SPTR(x) {&CF_AppData.config_table->x, sizeof(CF_AppData.config_table->x), NULL}
#define SPTRFN(x, fn) {&CF_AppData.config_table->x, sizeof(CF_AppData.config_table->x), fn}
#define CPTRFN(x, fn) {&CF_AppData.config_table->chan[chan_num].x, sizeof(CF_AppData.config_table->chan[chan_num].x), fn}
    const struct {
        void *ptr;
        uint32 size;
        int (*fn)(uint32, uint8 chan_num);
    } items[CF_NUM_CFG_PACKET_ITEMS] = {SPTR(ticks_per_second), SPTR(rx_crc_calc_bytes_per_wakeup), SPTR(ack_timer_s),
        SPTR(nak_timer_s), SPTR(inactivity_timer_s), SPTRFN(outgoing_file_chunk_size, CF_CmdValidateChunkSize),
        SPTR(ack_limit), SPTR(nak_limit), SPTR(local_eid), CPTRFN(max_outgoing_messages_per_wakeup, CF_CmdValidateMaxOutgoing) };
    ...
    if(param_id>=CF_NUM_CFG_PACKET_ITEMS) {
        CFE_EVS_SendEvent(CF_EID_ERR_CMD_GETSET_PARAM, CFE_EVS_EventType_ERROR, "CF: invalid configuration parameter id %d received", param_id);
        goto err_out;
    }

    if(chan_num>=CF_NUM_CHANNELS) {
        CFE_EVS_SendEvent(CF_EID_ERR_CMD_GETSET_CHAN, CFE_EVS_EventType_ERROR, "CF: invalid configuration channel id %d received", chan_num);
        goto err_out;
    }
    ...
    if(is_set) {
        ...
        if(items[param_id].fn) {
            if(!items[param_id].fn(value, chan_num)) {
                ...
                CFE_EVS_SendEvent(CF_EID_ERR_CMD_GETSET_VALIDATE, CFE_EVS_EventType_ERROR, "CF: validation for parameter id %d failed", param_id);
            }
        }
        else {
            valid_set = 1;
        ...
        if(valid_set) {
            CFE_EVS_SendEvent(CF_EID_INF_CMD_GETSET1, CFE_EVS_EventType_INFORMATION, "CF: setting parameter id %d to %d", param_id, value);
#if ENDIAN==_EB
        ...
        value >>= shift_map[items[param_id].size];
#endif
        CFE_EVS_SendEvent(CF_EID_INF_CMD_GETSET2, CFE_EVS_EventType_INFORMATION, "CF: parameter id %d = %d", param_id, value);
        acc=0;
    }
...
static void CF_CmdEnableEngine(CFE_SB_Buffer_t *msg)
{
    if(!CF_AppData.engine.enabled) {
        if(CF_CFDP_InitEngine()==CFE_SUCCESS) {
            ...
            CFE_EVS_SendEvent(CF_EID_ERR_CMD_ENABLE_ENGINE, CFE_EVS_EventType_ERROR, "CF: failed to re-initialize and enable CFDP engine");
            CF_CmdRej();
        }
    }
    else {
        CFE_EVS_SendEvent(CF_EID_ERR_CMD_ENG_ALREADY_ENA, CFE_EVS_EventType_ERROR, "CF: received enable engine command while engine already enabled");
        CF_CmdRej();
    }
}
...
static void CF_CmdDisableEngine(CFE_SB_Buffer_t *msg)
{
    if(CF_AppData.engine.enabled) {
        ...
        CFE_EVS_SendEvent(CF_EID_ERR_CMD_ENG_ALREADY_DIS, CFE_EVS_EventType_ERROR, "CF: received disable engine command while engine already disabled");
        CF_CmdRej();
    }
}
...
void CF_ProcessGroundCommand(CFE_SB_Buffer_t *msg)
{
    ...
    if(cmd<CF_NUM_COMMANDS) {
        ...
        if(len==expected_lengths[cmd]) {
            ...
            CFE_EVS_SendEvent(CF_EID_ERR_CMD_GCMD_LEN, CFE_EVS_EventType_ERROR, "CF: invalid ground command length for command 0x%02x, expected %d got %zd", cmd, expected_lengths[cmd], len);
            CF_CmdRej();
        }
    }
    else {
        CFE_EVS_SendEvent(CF_EID_ERR_CMD_GCMD_CC, CFE_EVS_EventType_ERROR, "CF: invalid ground command packet cmd_code=0x%02x", cmd);
        CF_CmdRej();
    }
}

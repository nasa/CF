...
static void CF_Chunks_EraseChunk(chunks_t *chunks, index_t erase_index)
{
    ...
    memmove(&chunks->chunks[erase_index], &chunks->chunks[erase_index+1], sizeof(*chunks->chunks)*(chunks->count-1-erase_index));
    --chunks->count;
}
...
static void CF_Chunks_InsertChunk(chunks_t *chunks, index_t index_before, const chunk_t *chunk)
{
    ...
    if(chunks->count&&(index_before!=chunks->count)) {
        memmove(&chunks->chunks[index_before+1], &chunks->chunks[index_before], sizeof(*chunk)*(chunks->count-index_before));
    }
    ...
    if(i!=chunks->count) {
        ...
        if(chunks->chunks[i].offset <= chunk_end) {
            ...
            if(combined_i!=chunks->count) {
                ...
                new_end = MAX(chunks->chunks[chunks->count-1].offset + chunks->chunks[chunks->count-1].size, chunk_end);
            }

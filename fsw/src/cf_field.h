...
#define inc_subfield(TYPE, field, mask, shift)  do { \
    TYPE old = *(field)&(mask); \
    old >>= (shift); \
    ++old; \
    old &= (mask); \
...
#define FIELD_GET_VAL(SRC, NAME) \
    (((SRC)>>(NAME).shift)&((NAME).mask))
...
#define FIELD_INC_VAL(DEST, NAME) \
    FIELD_ADD_VAL(DEST, NAME, 1)

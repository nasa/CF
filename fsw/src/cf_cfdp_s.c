...
{
    int32 ret = -1;
    pdu_header_t *ph = CF_CFDP_ConstructPduHeader(t, 0, CF_AppData.config_table->local_eid, t->history->peer_eid, 0, t->history->seq_num, 1);
    pdu_fd_t *fd;
    if(!ph) {
        ret = 0;
    ...
    if(t->state_data.s.cached_pos!=foffs) {
        ...
        if(status!=foffs) {
            CFE_EVS_SendEvent(CF_EID_ERR_CFDP_S_SEEK_FD, CFE_EVS_EventType_ERROR, "CF S%d(%u:%u): error seeking to offset 0x%08x, got 0x%08x", (t->state==CFDP_S2), t->history->src_eid, t->history->seq_num, foffs, status);
    ...
    if(status != bytes_to_read) {
        CFE_EVS_SendEvent(CF_EID_ERR_CFDP_S_READ, CFE_EVS_EventType_ERROR, "CF S%d(%u:%u): error reading bytes: expected 0x%08x, got 0x%08x", (t->state==CFDP_S2), t->history->src_eid, t->history->seq_num, bytes_to_read, status);
    ...
    if(status==CF_SEND_NO_MSG) {
        ...
        CFE_EVS_SendEvent(CF_EID_ERR_CFDP_S_SEND_FD, CFE_EVS_EventType_ERROR, "CF S%d(%u:%u): error sending fd", (t->state==CFDP_S2), t->history->src_eid, t->history->seq_num);
        goto err_out;
    }
    else {
...
static void CF_CFDP_S_SubstateSendMetadata(transaction_t *t)
{
    ...
    if(!OS_ObjectIdDefined(t->fd)) {
        ...
        if(OS_FileOpenCheck(t->history->fnames.src_filename)==OS_SUCCESS) {
            CFE_EVS_SendEvent(CF_EID_ERR_CFDP_S_ALREADY_OPEN, CFE_EVS_EventType_ERROR, "CF S%d(%u:%u): file %s already open", (t->state==CFDP_S2), t->history->src_eid, t->history->seq_num, t->history->fnames.src_filename);
        ...
        if(ret<0) {
            CFE_EVS_SendEvent(CF_EID_ERR_CFDP_S_OPEN, CFE_EVS_EventType_ERROR, "CF S%d(%u:%u): failed to open file %s, error=0x%08x", (t->state==CFDP_S2), t->history->src_eid, t->history->seq_num, t->history->fnames.src_filename, ret);
            ++CF_AppData.hk.channel_hk[t->chan_num].counters.fault.file_open;
        ...
        if(status<0) {
            CFE_EVS_SendEvent(CF_EID_ERR_CFDP_S_SEEK_END, CFE_EVS_EventType_ERROR, "CF S%d(%u:%u): failed to seek end file %s, error=0x%08x", (t->state==CFDP_S2), t->history->src_eid, t->history->seq_num, t->history->fnames.src_filename, status);
            ++CF_AppData.hk.channel_hk[t->chan_num].counters.fault.file_seek;
            goto err_out;
        }
        ...
        if(status!=0) {
            CFE_EVS_SendEvent(CF_EID_ERR_CFDP_S_SEEK_BEG, CFE_EVS_EventType_ERROR, "CF S%d(%u:%u): failed to seek begin file %s, got 0x%08x", (t->state==CFDP_S2), t->history->src_eid, t->history->seq_num, t->history->fnames.src_filename, status);
            ++CF_AppData.hk.channel_hk[t->chan_num].counters.fault.file_seek;
            goto err_out;
        }
    }
    ...
    if(sret==CF_SEND_ERROR) {
        /* failed to send md */
        CFE_EVS_SendEvent(CF_EID_ERR_CFDP_S_SEND_MD, CFE_EVS_EventType_ERROR, "CF S%d(%u:%u): failed to send md", (t->state==CFDP_S2), t->history->src_eid, t->history->seq_num);
        goto err_out;
    }
    else if(sret==CF_SEND_SUCCESS) {
...
static void CF_CFDP_S_SubstateSendFinAck(transaction_t *t)
{
    ...
    if(CF_CFDP_SendAck(t, ACK_TS_ACTIVE, PDU_FIN, t->state_data.s.s2.fin_cc, t->history->peer_eid, t->history->seq_num)!=CF_SEND_NO_MSG) {
        CF_CFDP_S_Reset(t);
    }
}
...
static void CF_CFDP_S2_EarlyFin(transaction_t *t, const pdu_header_t *ph)
{
    ...
    CFE_EVS_SendEvent(CF_EID_ERR_CFDP_S_EARLY_FIN, CFE_EVS_EventType_ERROR, "CF S%d(%u:%u): got early fin -- cancelling", (t->state==CFDP_S2), t->history->src_eid, t->history->seq_num);
    CF_CFDP_S_Reset(t);
}
...
static void CF_CFDP_S2_Fin(transaction_t *t, const pdu_header_t *ph)
{
    if(!CF_CFDP_RecvFin()) {
        ...
        CFE_EVS_SendEvent(CF_EID_ERR_CFDP_S_PDU_FIN, CFE_EVS_EventType_ERROR, "CF S%d(%u:%u): received invalid fin pdu", (t->state==CFDP_S2), t->history->src_eid, t->history->seq_num);
...
static void CF_CFDP_S2_Nak(transaction_t *t, const pdu_header_t *ph)
{
    ...
    if(!CF_CFDP_RecvNak(&num_sr)&&num_sr) {
        ...
        if(bad_sr) {
            CFE_EVS_SendEvent(CF_EID_ERR_CFDP_S_INVALID_SR, CFE_EVS_EventType_ERROR, "CF S%d(%u:%u): received %d invalid nak segment requests", (t->state==CFDP_S2), t->history->src_eid, t->history->seq_num, bad_sr);
        }
    }
    else {
        CFE_EVS_SendEvent(CF_EID_ERR_CFDP_S_PDU_NAK, CFE_EVS_EventType_ERROR, "CF S%d(%u:%u): received invalid nak pdu", (t->state==CFDP_S2), t->history->src_eid, t->history->seq_num);
        ++CF_AppData.hk.channel_hk[t->chan_num].counters.recv.error;
    }
}
...
static void CF_CFDP_S2_WaitForEofAck(transaction_t *t, const pdu_header_t *ph)
{
    if(!CF_CFDP_RecvAck()) {
        ...
        CFE_EVS_SendEvent(CF_EID_ERR_CFDP_S_PDU_EOF, CFE_EVS_EventType_ERROR, "CF S%d(%u:%u): received invalid eof pdu", (t->state==CFDP_S2), t->history->src_eid, t->history->seq_num);
        ++CF_AppData.hk.channel_hk[t->chan_num].counters.recv.error;
    }
}
...
static void CF_CFDP_S_DispatchRecv(transaction_t *t, void (*const fns[SEND_NUM_STATES][PDU_INVALID_MAX])(transaction_t*, const pdu_header_t*))
{
    ...
    if(!FGV(ph->flags, PDU_HDR_FLAGS_TYPE)) {
        ...
        if(fdh->directive_code<PDU_INVALID_MAX) {
            ...
            CFE_EVS_SendEvent(CF_EID_ERR_CFDP_S_DC_INV, CFE_EVS_EventType_ERROR, "CF S%d(%u:%u): received pdu with invalid directive code %d for sub-state %d", (t->state==CFDP_S2), t->history->src_eid, t->history->seq_num, fdh->directive_code, t->state_data.s.sub_state);
        }
    }
    else {
        CFE_EVS_SendEvent(CF_EID_ERR_CFDP_S_NON_FD_PDU, CFE_EVS_EventType_ERROR, "CF S%d(%u:%u): received non-file directive pdu", (t->state==CFDP_S2), t->history->src_eid, t->history->seq_num);
    }
}
...
void CF_CFDP_S1_Recv(transaction_t *t)
{
    ...
    static void (* const substate_fns[SEND_NUM_STATES][PDU_INVALID_MAX])(transaction_t*, const pdu_header_t*) = { {NULL} };
...
void CF_CFDP_S2_Recv(transaction_t *t)
{
    static void (* const substate_fns[SEND_NUM_STATES][PDU_INVALID_MAX])(transaction_t*, const pdu_header_t*) = {
        ...
        { NULL, NULL, NULL, NULL, NULL, CF_CFDP_S2_Fin, CF_CFDP_S2_WaitForEofAck, NULL, CF_CFDP_S2_Nak_Arm, NULL, NULL }, /* SEND_WAIT_FOR_EOF_ACK */
        { NULL, NULL, NULL, NULL, NULL, CF_CFDP_S2_Fin, NULL, NULL, CF_CFDP_S2_Nak_Arm, NULL, NULL }, /* SEND_WAIT_FOR_FIN */
...
void CF_CFDP_S_Tick(transaction_t *t, int *cont /* unused */)
{
    ...
    if(t->state==CFDP_S2) {
        if(CF_Timer_Expired(&t->inactivity_timer)) {
            CFE_EVS_SendEvent(CF_EID_ERR_CFDP_S_INACT_TIMER, CFE_EVS_EventType_ERROR, "CF S2(%u:%u): inactivity timer expired", t->history->src_eid, t->history->seq_num);
                ...
                if(CF_Timer_Expired(&t->ack_timer)) {
                    if(t->state_data.s.sub_state==SEND_WAIT_FOR_EOF_ACK) {
                        if(++t->state_data.s.s2.counter.ack==CF_AppData.config_table->ack_limit) {
                            CFE_EVS_SendEvent(CF_EID_ERR_CFDP_S_ACK_LIMIT, CFE_EVS_EventType_ERROR, "CF S2(%u:%u), ack limit reached, no eof-ack", t->history->src_eid, t->history->seq_num);
...
void CF_CFDP_S_Tick_Nak(transaction_t *t, int *cont)
{
    ...
    if(ret==1) *cont = 1; /* cause dispatcher to re-enter this wakeup */
}

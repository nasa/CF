...
#if ENDIAN==_EL
    for(i=size-1; i>0; --i) if(num[i]) break;
    ++i;
#elif ENDIAN==_EB
    for(i=0; i<(size-1); ++i) if(num[i]) break;
    i = size-i;
#else
#error define ENDIAN
#endif
...
static int CF_GetTSNSize(const pdu_header_t *ph)
{
    ...
    if(ret>sizeof(cf_transaction_seq_t)) {
        CFE_EVS_SendEvent(CF_EID_ERR_PDU_GET_TSN_SIZE, CFE_EVS_EventType_ERROR, "received TSN size %d too large for compiled max of %d", ret, (uint32)sizeof(cf_transaction_seq_t));
        return -1;
    }
...
static int CF_GetEIDSize(const pdu_header_t *ph)
{
    ...
    if(ret>sizeof(cf_entity_id_t)) {
        CFE_EVS_SendEvent(CF_EID_ERR_PDU_GET_EID_SIZE, CFE_EVS_EventType_ERROR, "received EID size %d too large for compiled max of %d", ret, (uint32)sizeof(cf_entity_id_t));
        return -1;
    }

...
;
;  Test Description
;	This procedure creates the initial CFS CCSDS File Delivery Protocol (CF)
;	Configuration Table load image file. This file is currently used by the
;	General Commanding test procedure.
;	
...
;	03/23/10	Walt Moleski	Inital implemetation.
...
$SC_$CPU_CF_TBLID = "CF Default Table"
...
;; Playback Channel 1
$SC_$CPU_CF_TBLPBCHAN[1].EntryInUse = CF_ENTRY_UNUSED
...
$SC_$CPU_CF_TBLPBCHAN[1].PollDir[1].CF_Class = 2
...
if (CF_MAX_POLLING_DIRS_PER_CHAN > 2) then
  for i = 2 to CF_MAX_POLLING_DIRS_PER_CHAN-1 do
    ...
    $SC_$CPU_CF_TBLPBCHAN[1].PollDir[i].PeerEntityID = ""
...
s create_tbl_file_from_cvt("$CPU", cfgAppid, "CF Default Configuration Table", "$cpu_cf_defcfg.tbl", cfgTblName, "$SC_$CPU_CF_TBLID[1]", endMnemonic)

%liv (log_procedure) = logging

write ";*********************************************************************"
write ";  End procedure $SC_$CPU_cf_tbl1"
write ";*********************************************************************"
ENDPROC

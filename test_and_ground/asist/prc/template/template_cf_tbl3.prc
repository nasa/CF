...
;	Configuration Table load image file to be used by the Queue Mangement
;	test procedure. This table has a Class 1 and a Class 2 enabled directory
;	to allow file playback under both modes. Also, this table contains a
;	unique message ID for each playback channel.
;	
...
;	07/19/10	Walt Moleski	Inital implemetation.
...
$SC_$CPU_CF_TBLPBCHAN[1].DnlinkPDUMsgID = CF_SPACE_TO_GND_PDU_MID + 1
...
s create_tbl_file_from_cvt("$CPU", cfgAppid, "CF Playback Configuration Table 3", "$cpu_cf_cfg3.tbl", cfgTblName, "$SC_$CPU_CF_TBLID[1]", endMnemonic)

%liv (log_procedure) = logging

write ";*********************************************************************"
write ";  End procedure $SC_$CPU_cf_tbl3"
write ";*********************************************************************"
ENDPROC
